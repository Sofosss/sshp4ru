...
use nix::sys::event::{Kqueue,KEvent, EventFilter, EventFlag, FilterFlag};
...
impl FdEvent {
    pub fn new(host: Rc<RefCell<Host>>, event_type: PipeType) -> Self {
        ...
        let mut fdev = FdEvent { host:  host.clone(), buffer: String::new(), offset: 0, fd: 0,  event_type: event_type };
    ...
    pub fn read_active_fd(&mut self, watcher: &Fdwatcher, last_host: &mut Option<String>, newline_print: &mut bool,
                          config_params: impl FnOnce() -> (bool, ProgMode, u16, u16, bool, bool)) -> Result<bool, RuntimeError> {
        let mut buffer = [0u8; 8192];
        let (silent,mode, max_line_length, max_output_length, anonymous_opt, colorize) = config_params();
        ...
        loop {
            match nix::unistd::read(fd, &mut buffer) {
                ...
                Ok(bytes_read) => {
                    ...
                    match mode {
                        ProgMode::Join => self.process_join_buf(&buffer[..bytes_read], max_line_length, max_output_length),
                        ProgMode::Group => {
                                            if let Err(_) = self.process_group_buf(&buffer[..bytes_read], &last_host, anonymous_opt, newline_print, colorize){
                                                return Err(RuntimeError::WriteStreamError);
                                            }
                                            *last_host = Some(self.host.borrow().name.clone());
                        },
                        ProgMode::Line => self.process_line_buf(&buffer[..bytes_read], max_line_length, anonymous_opt, colorize),
                    }
                },
    ...
    fn process_group_buf(&mut self, buffer: &[u8], last_host: &Option<String>, anonymous_opt: bool, newline_print: &mut bool, colorize: bool) -> io::Result<()> {
       ...
       //maybe somewhat ugly but gets rid of potential unsafe mutation on static last_host and newline_print
        ...
        let color = if ! colorize { Color::Empty.as_str() } else {
            match self.event_type {
    ...
    fn process_line_buf(&mut self, buffer: &[u8], max_line_length: u16, anonymous_opt: bool, colorize: bool) {
        // println!("{}", buffer.len());
        for ch in buffer.iter() {
    ...
    fn print_line_buffer(&self, anonymous_option: bool, colorize: bool) {
        ...
        let (color, cyan) = if ! colorize { (Color::Empty, Color::Empty) } else {
            (match self.event_type {
...
impl Fdwatcher {
    ...
    pub fn add(&mut self, monitor_fd: i32) -> io::Result<()> {
        ...
        let event = KEvent::new(monitor_fd as u64, EventFilter::EVFILT_READ, EventFlag::EV_ADD, FilterFlag::empty(), 0, 0);
    ...
    pub fn add(&self, monitor_fd: i32) -> io::Result<()> {
        ...
        if let Err(e) = epoll::ctl(self.epoll, epoll::ControlOptions::EPOLL_CTL_ADD, monitor_fd, event) {
            Err(e)
        }
        else {
            Ok(())
    ...
    #[cfg(feature = "USE_KQUEUE")]
    pub fn wait(&self, completed_events: &mut [RawFd], num_events: usize, timeout: i32) -> io::Result<()> {
        //not implemented yet. Probably doesn't work as expected
    ...
    #[cfg(not(feature = "USE_KQUEUE"))]
    pub fn wait(&self, completed_events: &mut [RawFd], num_events: usize, timeout: i32) -> Result<usize, RuntimeError> {    
    ...
    fn remove(&self, monitor_fd: i32) -> io::Result<()> {
        //not implemented yet. Probably doesn't work as expected
        self.kq.remove_fd(monitor_fd, kqueue::EventFilter::EVFILT_READ)?;
        Ok(())
    }
    ...
    fn remove(&self, monitor_fd: i32) -> Result<(), RuntimeError> {
        let event = epoll::Event::new(epoll::Events::EPOLLIN, monitor_fd as u64);
        if let Err(_) = epoll::ctl(self.epoll, epoll::ControlOptions::EPOLL_CTL_DEL, monitor_fd, event) {
